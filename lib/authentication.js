            ...
            switch(req.body.grant_type) {
              case 'password':
                if(req.body.username && req.body.password) {
                  var params = { username: req.body.username
                                ...
                                , client_id: client.id };
                  options.backend.passwordToken(req, function(err, responseVars) {
              ...
              case 'authorization_code':
                if(body.code) {
                  ...
                  options.backend.authToken(params, function(err, responseVars) {
                    if(err) { token_error(res, 'invalid_grant', 'Unable to authenticate token'); }
                    else { oauth_return(res, responseVars); }
                  });
                } else {
              ...
              case 'refresh_token':
                if(req.body.refresh_token){
                  ...
                  options.backend.refreshToken(params, function(err, responseVars) {
                    if(err) { token_error(res, 'invalid_grant', err); }
                    else { oauth_return(res, responseVars); }
                  });                  
                }
